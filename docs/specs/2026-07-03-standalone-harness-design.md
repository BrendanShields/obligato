# Design: Obligato Standalone Harness — Native Agent Runtime
...
`kernel/src/evaltask.ts`) or the cc-plugin hooks. This project makes Obligato a full
...
- Primary surface v1: interactive TUI chat (OpenTUI) plus `obligato run -p` headless
  print mode.
...
- `auth.ts` — `~/.obligato/auth.json` (0600); `api_key | oauth{access, refresh,
  expires}`; PKCE flow for Claude Pro/Max; single-flight refresh; injected into the
  Anthropic provider via custom `fetch`/headers; env-var fallback. `obligato auth login
  <provider>`. Highest external risk (unofficial flow) — isolated entirely here; API
  key always the fallback.
- `registry.ts` — shipped `models.json` + user overlay `~/.obligato/models.json`:
...
the loop to be Obligato's, not the SDK's.
...
- **Permissions:** ordered `{tool glob, arg glob, allow|ask|deny}` rules in
  `.obligato/permissions.json` (JSON, not YAML — no YAML dependency for a rules list); most-specific wins; per-agent overrides; "always allow"
...
3. **Every session is an eval:** session start writes a git-bundle snapshot;
   `obligato promote <session>` compiles a BenchmarkTask (statement = first user
...
4. **Tree sessions + replay:** `/tree` and `/fork [event-id]` over the parent_id DAG;
   `obligato session compare <headA> <headB>` diffs branch cost/outcomes; headless
...
|---|---|---|---|
| **6 Walking skeleton** | agent package (AI SDK wiring for Anthropic key + Ollama, step loop, 7 tools, minimal permissions, linear sessions on tree schema, cost registry), `obligato chat` + `obligato run -p` | Chat edits a real file; StepEvents land first-hand with correct micro-USD; `run -p --json` scripts | New spec doc: `agent-runtime.md` (AGT-*, PERM-*, SES-*, PROV-*); UX amendment |
...
| **8 Spec-native loop** | SpecContext, in-loop obligation runs (AGT-7 done-condition), ART-4 runtime gate reusing kernel `buildGate` (AGT-8), deterministic VerificationReport at session end (AGT-9) — the LLM clause-auditor gate is deferred to a later opt-in phase (Phase 8 decision: hermetic deterministic gate first) | Planted obligation violation blocks `done`; spec-less/stale T1 write blocked with override; VerificationReport emitted with failure_class | AGT-7..9; reuse kernel buildGate + compileSpec; no new event kind (obligation_check rides session_meta) |
| **9 Live routing + budget** | Per-step route/escalate (AGT-10), in-loop BudgetMonitor triage (AGT-11), obligation-fail escalation + T0 bandit (AGT-12); RoutingContext optional, kernel modules reused unchanged | Mechanical steps route cheap; a runaway fixture pauses at 2× with triage; obligation failure escalates the retry model | AGT-10..12 discharging RTR-*/RPOL-*/CTX-4 against the native loop |
| **10 Tree + session→eval** | session fork (SES-6), branch compare (SES-7), compaction (SES-8), session→BenchmarkTask promotion (EVP-10); replay = existing runEval over the promoted task (executor api) | Fork mid-session, compare branches; promote a session and replay under toggled lockfile to a verdict | SES-6..8 + EVP-10 |

## Hardest decisions (settled)

1. Provider layer → AI SDK, not hand-rolled: the SDK covers
   streaming/adapters/tools/usage; Obligato owns only auth (OAuth), pricing registry,
...
- Each phase's exit criteria are end-to-end checks run through the operator-facing
  surface (`obligato chat`, `obligato run -p`, `obligato eval ablate --executor api`).
