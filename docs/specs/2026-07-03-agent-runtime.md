...
- **AGT-6.** When a pause is recorded, the runtime shall require a non-empty reason string at record time — divergence ruling 2026-07-03: reader B's "pause reasons are validated at request time (non-empty string)" is adopted over reader A's "an empty-string reason is stored and returned as `paused("")`, not rejected". In Phase 6, paused state shall be derived from the pause-initiating event already enumerated in ERD §5 — a `permission_request` with no matching `permission_decision` — introducing no new event kind.
  *Obligation:* unit — an empty-string reason is rejected with a validation error and appends nothing; a paused fixture's derived state comes solely from the `permission_request`/`permission_decision` pair (kind inventory of the fixture's rows asserted).
