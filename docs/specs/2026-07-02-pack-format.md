...
**Capability enum (closed):** `stage:feedback`, `stage:ideation`, `stage:planning`, `stage:spec`, `stage:build`, `stage:verify`, `rules`, `routing-table`, `agent-registry`, `eval-suite`, `context-assembly`. A pack whose content directory implies a surface absent from `capabilities` is refused at load (SEC-4); adding a capability in an update is always a **major** version (§3). Note (ADR-0005, 2026-07-11): `context-assembly` content may **gate a built-in runtime subsystem** — a `context/lsp.yaml` enables the in-tree code-intelligence subsystem (AGT-17), which registers model-visible tools, widens the PERM-1 read-only default, and spawns a language-server subprocess — so approving a `context-assembly` declaration approves that reach; a dedicated `code-intelligence` capability value is deferred until a second gated subsystem exists (closed-enum extension under SEC-4, major under §3).
