...
import { DoctorReport } from "@obligato/schemas";
...
describe("UX-19: doctor names each failing component and its fix; healthy exits 0; never echoes credentials", () => {
  it("with no auth file, the auth component fails naming `obligato auth login <provider>`, exit non-zero, --json validates", async () => {
    const t = makeTestRepo({});
    mkdirSync(join(t.repo, ".obligato", "telemetry"), { recursive: true });
    ...
    expect(auth?.fix).toBe("obligato auth login <provider>");
  }, 30_000);

  it("a missing store fails naming `obligato init` and the probe never creates it (audit pin)", async () => {
    const t = makeTestRepo({});
    mkdirSync(join(t.repo, ".obligato", "telemetry"), { recursive: true });
    ...
    expect(store?.fix).toBe("obligato init");
  ...
  it("healthy fixture exits 0 with every component passing; output carries no credential substring", async () => {
    ...
    writeFileSync(
      join(t.home, ".obligato", "auth.json"),
